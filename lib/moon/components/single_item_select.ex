defmodule Moon.Components.SingeItemSelect.Item do
  ...
  def render(assigns) do
    ~F"""
    <div
      class={"p-2 pl-3 pr-3 hover:#{(@selected && "bg-piccolo-120") || "bg-hover"} rounded-md relative #{@selected && "bg-piccolo"}"}
      :on-click={@on_click}
...
defmodule Moon.Components.SingeItemSelect do
  ...
  def render(assigns) do
    ~F"""
    <div
      class={"bg-gohan shadow rounded-lg p-1 overflow-y-scroll #{@class}"}
      style={@style || ""}
    >
      <#slot />
