defmodule MoonWeb.Examples.Parts.ModalExample.WithForm do
  ...
  def mount(socket) do
    ...
    socket =
      case KycClient.get_merchants() do
        :error ->
          error_response(socket)
        ...
        {:ok, response} ->
          ...
          merchant_names =
            response.data
            |> Enum.map(fn merchant -> %{key: merchant.name, value: merchant.name} end)
          ...
          |> assign(:get_merchants_response, response.data)
