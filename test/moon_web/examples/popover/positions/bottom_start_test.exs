defmodule MoonWeb.Examples.Popover.Positions.BottomStartTest do
  use MoonWeb.ConnCase, async: true

  test "should be bottom-start popover", %{conn: conn} do
    {:ok, view, _html} = live(conn, "/example/PopoverExample.Position")

    assert view
           |> element("#popover-ex-bottom-start div.hidden[aria-expanded=false]")
           |> has_element?()

    refute view |> element("#popover-ex-bottom-start div[aria-expanded=true]") |> has_element?()

    assert view
           |> element("#popover-ex-bottom-start div.ltr\\:left-0.rtl\\:right-0.top-full")
           |> has_element?()
  end

  # TODO: add screenshot test of open panel, default and hover
end
