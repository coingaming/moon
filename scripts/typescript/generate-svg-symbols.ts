...
const getFiles = (iconType:string) =>  {
...
const camelToDashSnakeCase = (str:string) => str.replace(/[A-Z]/g, (letter:string, index:number) => { return index == 0 ? letter.toLowerCase() : '-'+ letter.toLowerCase();});
...
const getIntProp = (contents:string, propName:string):number =>
...
const writeSvgFile = (iconType:string, file:string, contents:string) => {
  ...
  if (iconType == "icons_new") {
    ...
    var svgPath = `${exportDir}/${iconType}/${file.toLowerCase()}.svg`.replace(
      /_/g,
      '-'
    );
  }
